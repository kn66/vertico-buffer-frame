...
Overlay layout derives its candidate frame from a golden rectangle.  Side-by-side
layout uses this factor when splitting the available width and when deriving
height from the parent frame."
  :type 'number
  :group 'vertico-buffer-frame)
...
(defun vertico-buffer-frame--side-by-side-tty-sizes (parent)
  "Return side-by-side candidate and preview sizes for TTY PARENT."
  (let* ((scale (vertico-buffer-frame--golden-scale))
         (parent-width (max 20 (frame-width parent)))
         (parent-height (max 1 (frame-height parent)))
         (gap (vertico-buffer-frame--side-by-side-gap-columns parent))
         (available-width (max 20 (- parent-width gap)))
         (scaled-width (max 20
                            (min available-width
...
(defun vertico-buffer-frame--side-by-side-graphic-sizes (parent)
  "Return side-by-side candidate and preview sizes for graphical PARENT."
  (let* ((scale (vertico-buffer-frame--golden-scale))
         (parent-width (max 20 (frame-width parent)))
         (parent-height (max 1 (frame-height parent)))
...
(defun vertico-buffer-frame--side-by-side-sizes (parent)
  ...
  (if (vertico-buffer-frame--tty-child-frames-p parent)
      (vertico-buffer-frame--side-by-side-tty-sizes parent)
    (vertico-buffer-frame--side-by-side-graphic-sizes parent)))
...
(defun vertico-buffer-frame--side-by-side-top (parent total-height)
  "Return side-by-side top display-unit position in PARENT."
...
(defun vertico-buffer-frame--child-frame-parameters ()
  ...
  (let* ((parent (selected-frame))
         ...
         (candidate-size (or (plist-get side-layout :candidate-size)
                             golden-size))
         (position (vertico-buffer-frame--candidate-position-from-size
                    parent candidate-size))
         (parameters
          (append
           (vertico-buffer-frame--compact-alist
            `((name . "vertico-buffer-frame")
              (parent-frame . ,parent)
              (width . ,(car candidate-size))
              (height . ,(cdr candidate-size))
           ...
           (vertico-buffer-frame--decoration-frame-parameters parent t))))
    parameters))
          ...
          (append
           (vertico-buffer-frame--compact-alist
            `((name . ,vertico-buffer-frame--preview-frame-name)
              (parent-frame . ,parent)
              (width . ,(car-safe preview-size))
              (height . ,(cdr-safe preview-size))
              (left . ,preview-left)
              (top . ,preview-top)
              (border-color . ,(vertico-buffer-frame--border-color))
              (background-color . ,(vertico-buffer-frame--background-color
           ...
           (vertico-buffer-frame--decoration-frame-parameters parent))))
    parameters))
...
(defun vertico-buffer-frame--candidate-top (parent candidate)
  "Return top display-unit position for the candidate child frame."
  (vertico-buffer-frame--center-leading-margin
   (frame-pixel-height parent)
