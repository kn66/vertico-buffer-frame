...
(ert-deftest vertico-buffer-frame-warmup-skips-non-graphic-display ()
  (let ((vertico-buffer-frame--warmed-up nil)
        made)
    (cl-letf (((symbol-function #'display-graphic-p)
               (lambda (&optional _frame) nil))
              ((symbol-function #'make-frame)
               (lambda (&optional _parameters)
                 (setq made t)
                 'warmup)))
      (should-not (vertico-buffer-frame-warmup))
      (should-not made)
      (should-not vertico-buffer-frame--warmed-up))))

(ert-deftest vertico-buffer-frame-warmup-creates-and-deletes-child-frame ()
  (let ((vertico-buffer-frame--warmed-up nil)
        made
        shown
        deleted)
    (cl-letf (((symbol-function #'display-graphic-p)
              ...
              ((symbol-function #'frame-visible-p)
               (lambda (_frame) t))
              ((symbol-function #'vertico-buffer-frame--base-parameters)
               (lambda (_parent name &optional size _role)
                 `((name . ,name) (width . ,(car size)))))
              ((symbol-function #'make-frame)
               (lambda (&optional parameters)
                 (setq made parameters)
                 'warmup))
              ((symbol-function #'vertico-buffer-frame--apply-border)
               #'ignore)
              ...
              ((symbol-function #'vertico-buffer-frame--delete-frame)
               (lambda (frame)
                 (setq deleted frame))))
      (should (vertico-buffer-frame-warmup))
      (should (equal (alist-get 'name made) "Vertico warmup"))
      (should (eq shown 'warmup))
      (should (eq deleted 'warmup))
      (should vertico-buffer-frame--warmed-up))))

(ert-deftest vertico-buffer-frame-warmup-deletes-frame-when-warmup-fails ()
  (let ((vertico-buffer-frame--warmed-up nil)
        deleted)
    (cl-letf (((symbol-function #'display-graphic-p)
               (lambda (&optional _frame) t))
              ((symbol-function #'frame-visible-p)
               (lambda (_frame) t))
              ((symbol-function #'vertico-buffer-frame--base-parameters)
               (lambda (_parent _name &optional _size _role) nil))
              ((symbol-function #'make-frame)
               (lambda (&optional _parameters) 'warmup))
              ((symbol-function #'vertico-buffer-frame--apply-border)
               (lambda (_frame)
                 (error "warmup failed")))
              ((symbol-function #'vertico-buffer-frame--delete-frame)
               (lambda (frame)
                 (setq deleted frame))))
      (should-error (vertico-buffer-frame-warmup))
      (should (eq deleted 'warmup))
      (should-not vertico-buffer-frame--warmed-up))))

(ert-deftest vertico-buffer-frame-mode-schedules-warmup ()
  (vertico-buffer-frame-test--with-clean-state
   (let ((vertico-buffer-frame--warmed-up nil)
         scheduled)
     (cl-letf (((symbol-function #'run-with-idle-timer)
                (lambda (secs repeat function &rest _args)
                  (setq scheduled (list secs repeat function))
                  'timer)))
       (vertico-buffer-frame-mode 1)
       (should (equal scheduled
                      (list 0 nil #'vertico-buffer-frame--warmup-startup)))
       (should (memq #'vertico-buffer-frame--warmup-startup
                     server-after-make-frame-hook))
       (vertico-buffer-frame-mode -1)
       (should-not (memq #'vertico-buffer-frame--warmup-startup
                         server-after-make-frame-hook))))))

(ert-deftest vertico-buffer-frame-warmup-startup-deregisters-when-mode-off ()
...
(ert-deftest vertico-buffer-frame-warmup-startup-deregisters-after-warmup ()
  (let ((vertico-buffer-frame-mode t)
        (vertico-buffer-frame--warmed-up nil))
    (add-hook 'server-after-make-frame-hook
              #'vertico-buffer-frame--warmup-startup)
    (cl-letf (((symbol-function
                #'vertico-buffer-frame--warmup-parent-frame)
               (lambda () 'parent))
              ((symbol-function #'vertico-buffer-frame-warmup)
               (lambda ()
