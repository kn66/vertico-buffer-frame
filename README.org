...
the lower-right side of the candidate frame.  Minibuffer-session frame cleanup is
delegated to Emacs' built-in ~minibuffer-exit~ frame parameter.
...
- ~vertico-buffer-frame-alpha~
- ~vertico-buffer-frame-alpha-background~
...
- ~vertico-buffer-frame-redraw-after-show~
...
#+begin_src elisp
  (setq vertico-buffer-frame-preview t
        ...
        vertico-buffer-frame-alpha 100
...
Embark collect, live, and export buffers still use Embark's normal display path,
so placement is controlled by ~display-buffer-alist~ and related Emacs display
settings.

When Embark is loaded, this package displays Embark buffers and Embark's
deferred display commands from the original non-minibuffer window instead of
the Vertico child frame.  Disable
~vertico-buffer-frame-embark-select-source-window~ to leave that context
untouched.
...
symbols, faces, Custom groups, environment variables, Unicode names, Info menu
items, calendar months and colors, text-like categories such as kill-ring,
dabbrev, ecomplete, email, BibTeX keys and expressions, and location-like
candidates from common completion categories such as Consult grep, Consult
compile errors, Consult Flymake diagnostics, Consult Info, imenu, Org headings
and xref.  Built-in Customize commands such as ~customize-option~,
~customize-face~, ~customize-group~ and ~customize-mode~ are also mapped to the
same preview functions even when their completion tables do not expose a
category.

When a completion table exposes Emacs' standard completion metadata, the
preview can also use its ~affixation-function~, ~annotation-function~, and
~group-function~ as a generic fallback.  This gives useful previews to more
built-in and third-party completion tables without adding package-specific
advice.
...
- ~vertico-buffer-frame~ avoids advising ~vertico-posframe~ and Consult preview
  ...
  and category-specific functions.  Completion categories are read through
  Emacs' built-in ~completion-metadata~ API.  ~vertico-posframe-preview~
  mirrors Consult preview state more directly, but it intentionally advises
  private APIs.
