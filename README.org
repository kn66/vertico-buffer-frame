...
centered child frame.  The implementation intentionally stays small: one fixed
candidate frame per minibuffer session, cleanup on minibuffer exit, and fallback
to the previous Vertico buffer display action when child frames are unavailable
or fail.
...
The frame size is fixed in character cells.  Additional frame parameters can be
added with ~vertico-buffer-frame-parameters~.
...
Consult-specific adapters, per-command local mode, golden-ratio sizing, or
